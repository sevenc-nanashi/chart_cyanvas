...
class BgmConvertJob < ApplicationJob
  ...
  def perform(bgm_resource)
    ...
    bgm_data =
      HTTP.get("#{ENV.fetch("SUB_AUDIO_HOST", nil)}/download/#{response[:bgm_id]}")
    ...
    HTTP.delete("#{ENV.fetch("SUB_AUDIO_HOST", nil)}/download/#{response[:bgm_id]}")

    logger.info "BgmConvertJob: #{bgm_resource.id}: downloading preview: #{response[:preview_id]}"
    preview_data =
      HTTP.get("#{ENV.fetch("SUB_AUDIO_HOST", nil)}/download/#{response[:preview_id]}")

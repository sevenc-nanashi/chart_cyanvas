...
class Api::ChartsController < FrontendController
  def all
    ...
    if params[:liked] == "true"
      ...
      cond[:id] = Like.where(user_id: user_id).pluck(:chart_id)
    end
    ...
    render json: { code: "ok", charts: charts }
  end
  ...
  def process_chart_request
    ...
    variant = nil
    unless data_parsed[:variant].blank?
      unless variant =
  ...
  def create
    ...
    unless (
             params.to_unsafe_hash.symbolize_keys in {
               data: String,
